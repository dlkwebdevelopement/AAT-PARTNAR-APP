...
const TruckRereg = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    //   !ton||
    //   !size||
    //   ownerAdharCard.length !== 2 ||
    //   ownerImage.length !== 1 ||
    //   ownerDrivingLicense.length !== 2 ||
    //   vehicleImages.length !== 5 ||
    //   vehicleInsurance.length !== 2 ||
    //   vehicleRC.length !== 2
    // ) {
    //   Toast.show({
    //     type: "error",
    //     text1: "Please fill in all required fields",
    //     text2: "Upload the required documents",
    //   });
    //   return;
    // }
    ...
    try {
      ...
      const res = await AxiosService.post(
        `vendor/editTruck/${vendorId}/${vehicleId}`,
        formData,
        {
          headers: {
      ...
      if (res.status === 201  ) {
        ...
        Toast.show({
          type: "success",
          text1: "Truck update successful!",
          text2: "Truck has been updated successfully.",
          position:"bottom"
        });
    setTimeout(() => {
        navigation.goBack();
