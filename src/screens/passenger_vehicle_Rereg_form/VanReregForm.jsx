...
const VanReregForm = ({ navigation, route }) => {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      const payload = {
        vendorId, vehicleId: vehicle._id, vehicleCategory: "vans",
        vehicleMake, vehicleModel, licensePlate, vehicleColor, numberOfSeats, milage, pricePerDay, pricePerKm, fuelType,
      ...
      if (res.status === 200 && res.data.razorpayOrderId) {
        const options = {
          description: "Van Re-registration",
  ...
  const verifyPayment = async (data) => {
    ...
    try {
      ...
      if (res.status === 200) {
        Toast.show({ type: "success", text1: "Van resubmitted!" });
  ...
  return (
    <View style={styles.main_container}>
      <View style={styles.header}><TouchableOpacity onPress={() => navigation.goBack()}><Icon name="left" size={24} /></TouchableOpacity><Text style={styles.headerTitle}>Resubmit Van</Text><View style={{width:24}}/></View>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {[
          ...
          { label: "Van Images (5)", state: vehicleImages, setter: setVehicleImages, limit: 5 },
        ...
        <Dropdown data={[{ label: "Diesel", value: "Diesel" }, { label: "Petrol", value: "Petrol" }]} onSelect={i => setFuelType(i.value)} placeholder="Fuel Type" />
