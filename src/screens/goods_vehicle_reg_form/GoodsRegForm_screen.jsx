...
const GoodsRegForm_screen = ({ navigation,route }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (goodsType === "Small" && (tonValue < 500 || tonValue > 1000)) {
      Toast.show({
        type: "info",
        text1: "Tonnage must be between 500 kg and 1000 kg",
        text2: "This category is only for 500 kg to 1000 kg",
      });
      return;
    } else if (goodsType === "Medium" && tonValue < 1000) {
      Toast.show({
        type: "info",
        text1: "Tonnage must be 1000 kg or above",
        text2: "This category is only for 1000 kg to 10,000 kg",
      });
      return;
    } else if (goodsType === "Large" && tonValue < 10000) {
      Toast.show({
        type: "info",
        text1: "Tonnage must be 10000 kg or above",
        text2: "This category is only for 10000 kg to 20,000 kg",
      });
      return;
    } else if (goodsType === "XL" && tonValue < 20000) {
      Toast.show({
        type: "info",
        text1: "Tonnage must be 20000 kg or above",
        text2: "This category is only for more than 20000 kg",
      });
      return;
    }
    ...
    try {
      ...
      const res = await AxiosService.post("vendor/createTruck", payload);
      ...
      <View style={styles.content_container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.form_heading_container}>
            ...
            />
            {/* form heading text */}
            <Text style={styles.form_heading_txt}>
              {/* Goods Vehicle Registartion Form */}
            </Text>
          </View>
          {/* heading text */}
        ...
        <View style={styles.input_field_container}>
          <Text style={styles.label}>
            Vendor Image{" "}
            <Text style={styles.subText}>( minimum 1 image )</Text>
          </Text>
          <Pressable
        ...
        <View style={styles.input_field_container}>
          <Text style={styles.label}>
            Vehicle Image{" "}
            <Text style={styles.subText}>( minimum 5 images )</Text>
          </Text>
          <Pressable
            style={styles.container}
        ...
        <View style={styles.input_field_container}>
          <Text style={styles.label}>
            License <Text style={styles.subText}>( front and back )</Text>
          </Text>
          <Pressable
            style={styles.container}
            onPress={() =>
        ...
        <View style={styles.input_field_container}>
          <Text style={styles.label}>
            Insurence <Text style={styles.subText}>( minimum 2 images )</Text>
          </Text>
          <Pressable
            style={styles.container}
            onPress={() =>
    ...
    onChangeText={(text) => {
      ...
      if (year.length <= 4) {
        if (parseInt(year) > currentYear) {
          Toast.show({
            type: "error",
            text1: `Vehicle make year cannot be greater than ${currentYear}`,
          });
  ...
  />
        ...
        <View style={styles.input_field_container}>
        <Text style={styles.label}>Vehicle Tonnage Capacity in Kg</Text>
<TextInput
  keyboardType="numeric"
  value={ton}
  onChangeText={(text) => {
    let filteredText = text.replace(/[^0-9]/g, ''); // Remove non-numeric characters
    if (goodsType === 'Small' && Number(filteredText) > 1000) {
      filteredText = '1000'; // Limit the value to 1000
    }else if(goodsType === 'Medium' && Number(filteredText) > 10000){
      filteredText = '10000';
    }
    else if(goodsType === 'Large' && Number(filteredText) > 20000){
      filteredText = '20000';
    } 
    setTon(filteredText);
  }}
  style={styles.input_field}
  placeholder="Ex: 1 Ton = 1000 kg"
/>
</View>
          {/* vehicle size */}
          <View style={styles.input_field_container}>
...
<Dropdown
        ...
        customStyles={styles.customCheckbox} // Optional custom styles
      />
