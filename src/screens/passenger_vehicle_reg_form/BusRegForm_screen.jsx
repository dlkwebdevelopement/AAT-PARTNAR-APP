...
const BusRegForm_screen = ({ navigation }) => {
    ...
    try {
      ...
      if (validDocuments.length > 0) {
        if (currentDocuments.length + validDocuments.length > limit) {
          Alert.alert("Document Limit Exceeded", `You can only upload up to ${limit} document(s).`);
          return;
        }
        const base64Documents = await Promise.all(
          validDocuments.map(async (doc) => ({
            ...doc,
            base64: await convertToBase64(doc),
          }))
        );
        setDocumentState((prev) => [...prev, ...base64Documents]);
      } else {
        Alert.alert("Invalid File Type", "Please select an image (JPG, PNG, etc.) or PDF file.");
      }
    } catch (error) {
      ...
      return (
        <View key={index} style={styles.doc_container}>
          ...
          <Pressable
            ...
            onPress={() => setDocumentState((prev) => prev.filter((_, i) => i !== index))}
          >
  ...
  const buildPayload = () => ({
    ...
    numberOfSeats, milage, ac, pricePerDay, pricePerKm, fuelType,
  ...
  const resetForm = () => {
    ...
    setNumberOfSeats(""); setMilage(""); setAc(""); setPricePerDay(""); setPricePerKm("");
  ...
  const handleSubmit = () => {
    ...
    if (minPrice !== null && kmVal < minPrice) {
      Toast.show({
        ...
        text2: `Minimum allowed for Bus is ₹${minPrice}/km`,
      });
      return;
    }
    if (maxPrice !== null && kmVal > maxPrice) {
      Toast.show({
        ...
        text2: `Maximum allowed for Bus is ₹${maxPrice}/km`,
      });
      return;
    }
    ...
    try {
      ...
      if (res.status === 200 && res.data.razorpayOrderId) {
        ...
        RazorpayCheckout.open(options)
          ...
          .catch((error) => {
            if (error.code === 2) Toast.show({ type: "info", text1: "Payment cancelled" });
            else Toast.show({ type: "error", text1: "Payment failed", text2: error.description });
          });
      }
    } catch (error) {
      Toast.show({ type: "error", text1: error.response?.data?.message || "Failed to initiate payment." });
    } finally {
      setLoading(false);
    }
  };
      ...
      <View style={styles.content_container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={styles.form_heading_container}>
            <Image source={require("../../assets/Images/bus.png")} style={styles.heading_img} />
            <Text style={styles.form_heading_txt}>Bus Registration Form</Text>
          </View>
          ...
          <View style={styles.input_field_container}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                ...
                const currentYear = new Date().getFullYear();
                if (year.length <= 4) {
                  if (parseInt(year) > currentYear) { Toast.show({ type: "error", text1: `Year cannot be greater than ${currentYear}` }); setVehicleMake(currentYear.toString()); }
          ...
          <View style={styles.input_field_container}>
            ...
            <TextInput value={numberOfSeats} onChangeText={setNumberOfSeats} style={styles.input_field} />
          </View>

          <View style={styles.input_field_container}>
            <Text style={styles.label}>AC</Text>
            <TextInput value={ac} onChangeText={setAc} style={styles.input_field} placeholder="Ex: Yes or No" />
          </View>
          ...
          <View style={styles.input_field_container}>
            <Text style={styles.label}>
              ...
              {minPrice !== null && maxPrice !== null && (
                <Text style={styles.price_range_hint}>(₹{minPrice} – ₹{maxPrice}/km allowed)</Text>
              )}
            </Text>
            <TextInput
          ...
          <TouchableOpacity
            ...
            style={[styles.btn_container, (registerAmount === null || !!pricePerKmError) && styles.btn_disabled]}
            activeOpacity={0.85} disabled={registerAmount === null || !!pricePerKmError}
          >
            {registerAmount === null ? <ActivityIndicator color={colors.white} /> : (
              <>
      ...
      <Modal visible={showPaymentConfirm} transparent animationType="slide" onRequestClose={() => setShowPaymentConfirm(false)}>
        <View style={styles.modal_overlay}>
          <View style={styles.modal_box}>
            ...
            <Text style={styles.modal_subtitle}>Bus Registration Fee</Text>
...
export default BusRegForm_screen;
